...
from pymongo.synchronous.command_cursor import CommandCursor as SyncCommandCursor
from pymongo.synchronous.cursor import Cursor as SyncCursor

if not hasattr(SyncCursor, "_Cursor__die"):
    SyncCursor._Cursor__die = SyncCursor._die_lock

if not hasattr(SyncCursor, "_Cursor__query_flags"):
    SyncCursor._Cursor__query_flags = property(lambda self: self._query_flags)

if not hasattr(SyncCursor, "_Cursor__data"):
    SyncCursor._Cursor__data = property(lambda self: self._data)

if not hasattr(SyncCursor, "_Cursor__killed"):
    SyncCursor._Cursor__killed = property(lambda self: self._killed)

if not hasattr(SyncCommandCursor, "_CommandCursor__die"):
    SyncCommandCursor._CommandCursor__die = SyncCommandCursor._die_lock
